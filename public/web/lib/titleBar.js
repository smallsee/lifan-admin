    ...
    TitleBarXiaohai.prototype = {
      ...
      init : function(){
        ...
        for(var i=0;i<me.leftItem.data.length;i++){
            ...
            liItem = $('<li page='+i+' class='+me.leftItem.liClass+i+'>').css({
          ...
          if(me.leftItem.data[i].href.length>0){
            var titleItem = $('<a href='+me.leftItem.data[i].href+' class='+me.leftItem.AClass+[i]+'>').css({
            ...
            if(me.leftItem.data[i].icon.length>0 || me.leftItem.data[i].hasIcon){
              var titleIcon = $('<span class='+me.leftItem.iconClass+[i]+'>').html(me.leftItem.data[i].icon).addClass(me.leftItem.data[i].iconClass);
      ...
      animated : function(){
        var me = this;
//mouse
        if (me.settings.isMouse){
          me.mouseout = $('<div class='+me.settings.mouse.class+'>').css({
            position:'absolute',
            width:me.leftLiStyle.width,
            height:me.leftLiStyle.height,
            backgroundColor:me.settings.mouse.color,
            zIndex:-2,
            top:(me.leftUlStyle.height-me.leftLiStyle.height)/2,
            borderRadius:me.settings.mouse.radius
          });

          me.leftUl.append(me.mouseout);

          me.leftLi =  me.leftUl.children('li');
          me.leftLi.on('mouseover',function(){
            var page = $(this).attr('page');

            me.mouseout.animate({
              left:page*(me.leftLiStyle.width+me.leftLiStyle.marginRight)
            },300)
          })
